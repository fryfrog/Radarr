            ...
            if (Settings.MovieMetadata)
            {
                ...
                using (var xw = XmlWriter.Create(sb, xws))
                {
                    ...
                    if (movieFile.MediaInfo != null)
                    {
                        ...
                        var people = _personService.GetAllPeopleForMovie(movie.Id);

                        foreach (var person in people)
                        {
                            if (person.Name != null && person.CastCharacter != null)
                            {
                                var actorElement = new XElement("actor");

                                actorElement.Add(new XElement("name", person.Name));
                                actorElement.Add(new XElement("role", person.CastCharacter));
                                actorElement.Add(new XElement("order", person.Order));

                                var headshot = person.Images.FirstOrDefault(m => m.CoverType == MediaCoverTypes.Headshot);
